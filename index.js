...
*/
var	timers = {};//保存计时器, 用于在没有可用客服时, 继续查找 可用的客服
var	status ={
		//会话状态
		CONNECTING:0,//连接服务器中
		CONNECTED:1,//连接服务器成功
...
io.on('connection', function (socket) {
	...
	//连接成功，重新匹配客服，防止客服刷新页面，用户界面掉线
	socket.on('relink',addUser);
	...
	socket.on('stop typing',updateTyping);
});
...
function updateTyping(data){
	...
	target=user.target;
	if(target){
		target.socket.emit(data.evt,{username:target.name});
	}
}
...
function addUser(data){
	...
	if(data.relink){
		...
		return  this.emit('log',{type:0,msg:'正在为您再次转接客服，请稍后... '})
...
function removeUser(){
	...
	if(target){
		switch(user.type){
			case 1://客服
				if(target.socket){
					target.socket.emit('log', {status:status.LINKING,msg:'客服已退出会话!'});
