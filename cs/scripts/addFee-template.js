$(function(){
    ...
    tep.init = function(){
        ...
        $(".pay_list_c2").on("click",function(){
          var _this = $(this), value = parseInt(_this.find("input").val());
          _this.addClass("on").siblings().removeClass("on");
          var $jDefault = $(".J_PostageDetail"); $jDefaultSet = $jDefault.find(".J_DefaultSet");
          switch(value){
             case 1:
                $jDefaultSet.find("span").eq(0).text("件内");
                $jDefaultSet.find("span").eq(1).text("内");
                if($jDefault.find("table").length > 0){
                    $jDefault.find("table").find("th").eq(1).text("首件(件)");
                    $jDefault.find("table").find("th").eq(3).text("续件(件)");
                }
                break;
             case 2:
                $jDefaultSet.find("span").eq(0).text("kg内");
                $jDefaultSet.find("span").eq(1).text("kg");
                if($jDefault.find("table").length > 0){
                    $jDefault.find("table").find("th").eq(1).text("首重(kg)");
                    $jDefault.find("table").find("th").eq(3).text("续重(kg)");
                }
                break;
             case 3:
                $jDefaultSet.find("span").eq(0).text("m³内");
                $jDefault.find("span").eq(1).text("m³");
                if($jDefault.find("table").length > 0){
                    $jDefault.find("table").find("th").eq(1).text("首体积(m³)");
                    $jDefault.find("table").find("th").eq(3).text("续体积(m³)");
                }
                break;
             default:
                break;
        ...
        $(".J_AddRule").click(function(){
            var html = "";
            var _this = $(this), vtype = parseInt($(".pay_list_c2.on").find("input").val());
    ...
    tep.addCity = function(){
        ...
        var url = "http://goods.qbao.com/postage/city.html";
